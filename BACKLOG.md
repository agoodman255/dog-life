...
| 18 | Unify scheduled routines with the Quick log (expectation ↔ observation) | High | Very High | Built | 🟡 **Built and browser-verified 2026-07-27, all 7 phases.** Migrations **not yet run against production** — see item 18 notes. |
...
### 18. Unify scheduled routines with the Quick log
Came up 2026-07-27. Andrew: adding known items to the calendar (potty break morning, breakfast, potty after breakfast…) gave a completely different experience from Quick-logging the same thing on the Dashboard, and the two never reconciled — "these categories in the calendar should have the same or very similar logging experience."

**The diagnosis.** A scheduled routine is the *expectation* ("a potty break should happen around 7:15"); a Quick log is the *observation* ("one happened at 7:18 — poo, outside, firm"). Same event, two moments. The app modelled them with two vocabularies — a hand-typed checklist on one side, `QUICK_LOG_SPECS` on the other — and the database actively forbade joining them (`check ((item_id is not null) <> (quick_log_kind is not null))`). Logging a potty left the 7:15 slot `not_started` forever.

**What the data is for**, which is the half that justified the capture: vet-ready health history (stool/urine over 30 days); housetraining progress measured as *interval between breaks widening while accidents fall* — only computable once expectation and observation are joined; routine adherence, i.e. which routines are real and which are calendar decoration; and better input to the `review-logs` ingest. Nothing was added without one of those consuming it.

**Decisions Andrew made** (2026-07-27): auto-match **proposes and confirms, never silently attaches** — one candidate gets "Looks like this is Morning potty (7:15 AM). Is it?", multiple get a list, and the app never picks between two; calendar noise handled by a **per-viewer toggle first, stored per-item flag second**; full scope including history views.

**What shipped, in seven phases:**
- **"Hide routines" calendar toggle** — per-viewer, persisted like the day-display preference, default on. `recurring`/`upcoming` stay unfiltered so nothing is ever unreachable. This is Bree's ask from 2026-07-12 (FEEDBACK.md) and it ships standalone.
- **One form everywhere.** `CATEGORY_QUICK_LOG_KIND` maps potty→potty, meals→food, training→training, exercise→play. Those categories open `QuickLogForm` (kind locked, dogs and milestone pre-seeded) instead of the generic `LogEntryPanel`, which stays unchanged for health categories where `DEFAULT_LOG_FIELDS` earns its keep. Mapped categories offer logging even when `requiresLog` is off — for a potty break, recording what happened *is* how you say it happened.
- **The link.** Constraint relaxed to "at least one of item_id/quick_log_kind". New columns: `item_logs.happened_at` (a 7:18 break entered at 9pm was reading as 9pm, which broke matching, ordering and every interval calculation), `item_occurrences.satisfied_by_log_ids`, `items.calendar_visibility`.
- **"Counts as."** ±90-minute window (under half the gap in the densest realistic schedule), candidates filtered by category, recurrence, open state and dog overlap, sorted nearest-first. **Partial dog coverage does not complete**: an occurrence has one state for all dogs, so a log covering only Mara leaves the slot open reading "Still expected: Griz" until a second log closes it. Occurrence rating takes the *minimum* across claiming logs. Deleting or editing a log releases exactly what it claimed, with a history entry naming why.
- **Merged Dashboard "Today."** One time-ordered list replacing the disconnected Agenda and Log panels: expected-open, expected-satisfied (log summary inline), and unscheduled-and-logged. Header reads "N logged · M still expected". Drops `useAdaptivePlan`'s optional-item hiding, which is defensible for a coaching agenda and a lie for a list claiming to be the whole day.
- **Routine cleanup.** Seed routines carry `calendarVisibility`; `morning-potty`'s hand-typed checklist (Peed/Pooped counters, playtime, treats) is deleted as a worse copy of the potty spec. The stored flag fixed what the heuristic got wrong — "Parallel decompression walk" at 6:15pm belongs on a calendar and is now back on it.
- **History.** Per-dog daily record in Health (7/30/90 days): average and longest gap between breaks, days since last accident, best clean run, stool/urine/appetite/water distributions. Analytics gains routine adherence worst-first, and its hardcoded `"26/day"` training-minutes string is now a real figure.

**Still open:**
- ~~Neither migration has been run against production.~~ **Run 2026-07-27** — Andrew ran `schema.sql` (a superset of both migration files) against production directly.
- ~~`item_logs.item_id` is still `on delete cascade`...~~ **Closed 2026-07-27.** A `before delete` trigger on `items` (`detach_quick_logs_before_item_delete`, `schema.sql`) now does the same detach at the database level, so a delete issued directly against the database (Supabase table editor, a manual `delete from items`) can't bypass it either. The app-side detach in `deleteItem` (`store.tsx`) is now belt-and-braces rather than the only guard.
- Partly subsumes **item 12** (daily training auto-scheduling) by making "did the training around the meal actually happen" measurable for the first time — the rule engine now has real adherence data to reason about instead of guessing.
- Tasks view's delete was a bare `items.remove`, skipping both the required deletion note and the new detach. Now routed through the same `DeleteEventModal` as everywhere else.

**Follow-up, 2026-07-27 (Phase 7): widened category→kind overlap.** Andrew found "counts as" working for Potty and Training but not Play, Food, or Water. Traced precisely: the map itself was already correct (`meals → food`, `exercise → play`), but (a) the only seed item with `category: "exercise"` is a one-off calendar event, hardcoded `requiresCompletion: false`, so it could never be a candidate — the routine that's actually walk-shaped, "Parallel decompression walk," carries `category: "relationship"` instead, which wasn't mapped at all; (b) `water` has no `Category` at all, by design; (c) Food was actually fine mechanically, just likely tested outside the ±90-minute window around the two meal times.

Andrew's calls: map `relationship → play` too (the walk is the only completable physical-activity routine that exists); and rather than leaving Water permanently unscheduled or adding a dedicated Water calendar routine, let `meals` satisfy **both** Food and Water — one calendar slot, feed and top up water at the same visit.

**What shipped:** `CATEGORY_QUICK_LOG_KIND` changed from `Category → QuickLogKind` to `Category → QuickLogKind[]` (`utils.ts`), now `meals: ["food", "water"]`, `exercise: ["play"]`, `relationship: ["play"]`. `quickLogKindForCategory` (singular) kept its exact signature and "primary kind" meaning, so all six existing call sites needed zero changes — a meal's Log button still defaults to Food. New `quickLogKindsForCategory` (plural) feeds `scheduledSlotCandidates`'s matching filter, the only place matching logic itself changed. `QuickLogForm` gained one new render branch: when attached to an item whose category covers more than one kind, the kind choice stays live (a small `.subtabs` toggle, not the full 5-way picker) instead of collapsing to a locked label, so opening "Breakfast" lets you flip to Water without leaving the attachment.

**Known, accepted consequence:** if both a Food and a Water log attach to the same meal occurrence, the occurrence's single top-level rating becomes the *minimum* of the two (the existing weakest-link rule). Considered and kept rather than splitting meals into separate Food/Water calendar items — each individual log's own rating stays visible in `TaskCard`'s attached-logs list regardless, the blending only affects the one headline number, and the rule mirrors the per-checklist-row-vs-overall pattern Andrew already chose for training sessions (item 11). Revisit if the blended number turns out to mislead in practice.

No schema change — `CATEGORY_QUICK_LOG_KIND` is a pure code-level lookup, never persisted.
