...
export const item = {
  fromRow(row: any): Item {
    return {
      ...
      calendarVisibility: row.calendar_visibility ?? "calendar",
  ...
  toRow(entry: Item, householdId: string) {
    return {
      ...
      calendar_visibility: entry.calendarVisibility ?? "calendar",
...
export const itemLog = {
  fromRow(row: any): ItemLog {
    return {
      ...
      // Rows written before happened_at existed fall back to when they were saved,
      // which is the best available answer and what the column backfill uses too.
      happenedAt: row.happened_at ?? row.logged_at,
  ...
  toRow(entry: ItemLog, householdId: string) {
    return {
      ...
      happened_at: entry.happenedAt ?? entry.loggedAt,
